...
@section('title', 'Edit Gambar Galeri - Kosku')
...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-[#1b1b18] dark:text-[#EDEDEC] mb-2">Edit Gambar Galeri</h1>
        <p class="text-[#706f6c] dark:text-[#A1A09A]">Perbarui informasi gambar galeri</p>
    </div>
    ...
    <div class="bg-white dark:bg-[#161615] border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg shadow-sm p-6 md:p-8">
        <form action="{{ route('galleries.update', $gallery) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <div class="w-full max-w-xs aspect-square bg-gray-200 dark:bg-gray-800 rounded-lg overflow-hidden">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" 
                          id="description" 
                          rows="3"
                          class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-sm bg-white dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC] focus:outline-none focus:ring-2 focus:ring-[#f53003] dark:focus:ring-[#FF4433] focus:border-transparent">{{ old('description', $gallery->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
